...
def _validate_flood_polygons(
    ...
    if flood_polygons["source"]["source_type"] != "modeled_input":
        raise ScenarioValidationError("Flood polygons must identify modeled input provenance")
    ...
    for feature in features:
        ...
        bands_by_frame[frame_id].append((depth_min, depth_max))
    ...
    for frame_id, bands in bands_by_frame.items():
        if not bands:
            raise ScenarioValidationError(
                f"Flood polygons must include at least one band for {frame_id}"
            )
